package pro.sky.sheltertelegrambot.controllers;
...
import pro.sky.sheltertelegrambot.services.MessageToVolunteerService;
...
public class MessageToVolunteerController {
    ...
    @Operation(
            ...
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "Все объекты MessageToVolunteer с полем answer = null",
                            content = @Content(
    ...
    public void writeInFieldAnswer(@Parameter(description = "Идентификатор объекта MessageToVolunteer")
                             @PathVariable("id") int id,
                             @Parameter(description = "Ответ волонтера")
                             @RequestParam String answer,
                             @RequestParam boolean replyToMessage)
    {
        messageToVolunteerService.writeAnswer(id, answer, replyToMessage);
    }
}
